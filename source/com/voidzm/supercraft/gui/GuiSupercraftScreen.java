...
public class GuiSupercraftScreen extends GuiScreen {
	...
	public void drawScreenBackground(int mouseX, int mouseY, float tick) {
		...
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, this.mc.renderEngine.getTexture("%blur%/mods/supercraft/textures/gui/bg"+(index == 0 ? "" : index+1)+".png"));
		...
		if(progress >= (imageTime-transitionTime)) {
			...
			GL11.glBindTexture(GL11.GL_TEXTURE_2D, this.mc.renderEngine.getTexture("%blur%/mods/supercraft/textures/gui/bg"+(indexAdj == 0 ? "" : indexAdj+1)+".png"));
