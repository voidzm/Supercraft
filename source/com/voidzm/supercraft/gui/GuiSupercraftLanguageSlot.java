...
public class GuiSupercraftLanguageSlot extends GuiSupercraftSlot {

	private final GuiSupercraftLanguage parent;
	
	private ArrayList arrayList;
	private TreeMap treeMap;

	public GuiSupercraftLanguageSlot(GuiSupercraftLanguage screen) {
	...
	protected void elementClicked(int par1, boolean par2) {
		...
		GuiSupercraftLanguage.getGameSettings(this.parent).language = (String)this.arrayList.get(par1);
		this.parent.getFontRenderer().setBidiFlag(StringTranslate.isBidirectional(GuiSupercraftLanguage.getGameSettings(this.parent).language));
		GuiSupercraftLanguage.getDoneButton(this.parent).text = StringTranslate.getInstance().translateKey("gui.done");
		GuiSupercraftLanguage.getGameSettings(this.parent).saveOptions();
	}
	...
	protected void drawSlot(int par1, int par2, int par3, int par4, Tessellator par5Tessellator) {
		...
		ScaledResolution res = new ScaledResolution(GuiSupercraftLanguage.getGameSettings(this.parent), parent.getMinecraft().displayWidth, parent.getMinecraft().displayHeight);
