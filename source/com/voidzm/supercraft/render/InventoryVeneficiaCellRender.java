...
public class InventoryVeneficiaCellRender implements IItemRenderer {
	...
	private void bindMaterialForIndex(int index) {
		...
		ResourceLocation resource = new ResourceLocation("supercraft:textures/items/veneficiacell_"+VeneficianProperties.materialResourceNames[index]+".png");
		tm.func_110577_a(resource);
	}
	
	private void bindVeneficiaForIndex(int index) {
		if(index == 16) return; // Empty cell
		TextureManager tm = Minecraft.getMinecraft().renderEngine;
		ResourceLocation resource = new ResourceLocation("supercraft:textures/items/veneficia_"+VeneficianProperties.aspectNames[index].toLowerCase()+".png");
		tm.func_110577_a(resource);
	}
